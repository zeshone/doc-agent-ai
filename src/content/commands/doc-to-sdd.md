...
**Prerequisites:** At least one business-layer artifact (`_requirements.md` or `_prd.md`) OR at least one technical-layer artifact (`_tech-spec.md` or `_db-design.md`) must exist under `<BASE_PATH><system>/`.

**Output:** LLM-optimized SDD context files written to `<BASE_PATH><system>/agent_sdd_context_project/`:
