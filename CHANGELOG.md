...
Vault users never received the skill built to read compacted context, because it hardcoded a path that only exists in-project mode. Six other skills kept instructing the agent to write files the program had already taken ownership of writing. And the gate meant to keep documentation content in English had been reporting success without reading a single line since before v5.0.0 shipped — which is how a corrupted trigger phrase reached that release undetected. Nothing here breaks a contract: every addition is optional, and every fix restores behavior the project already promised.

### Added

- **`.doc-agent.json` can record which documentation node a project belongs to.** Vault mode resolves a node's documentation under `<vault>/<node>/`, and nothing in a code repository recorded which node it mapped to — an agent working in the repo had no way to name its own node, and a tool that cannot name the node cannot ask the program for paths. The marker's new `node` key is additive; `status --node` now falls back to it when the flag is omitted, while an explicit `--node` still wins. No other node-taking command changed.
- **`doctor` adopts a compacted SDD context written before the manifest existed.** A pre-v5 `doc-to-sdd` wrote its two context files with no node prefix and no manifest. In vault mode the program only ever looked for the prefixed name, so that documentation sat on disk unreachable — three of four modules in one real vault were in this state, reporting absent. `doctor` now recognises the legacy filenames, the way it already recognises legacy phase artifacts, and records them under a new `adopted` state: present and usable, coverage explicitly unverified. It renames nothing on disk and writes no manifest for them — hashing today's sources would claim the compaction saw bytes it never saw.
- **`status` exposes a node's `children`.** The data already existed — the same lookup that renders a system's module table into its index — it simply never reached the JSON, so `doc-reader` had no way to enumerate a system's modules and had to ask the human for a node identifier, syntax included. `children` carries identity only, never a child's own status: a caller that needs that asks `status` for the child directly, rather than depending on the markdown its index happens to render.

### Fixed

- **A corrupted exclusion trigger shipped in v5.0.0.** The `doc-arch` skill's Spanish "skip the database design" trigger read `no我们需要base de datos` — Chinese for "we need" spliced into the middle of a Spanish phrase, inverting the sense of the thing it was meant to trigger on. It went uncaught because the CI gate meant to catch exactly this kind of corruption had never actually run one line of the codebase (see below). Restored to `no necesitamos base de datos`. Anyone who installed v5.0.0 has this corrupted phrase in their installed `doc-arch` skill today; reinstalling picks up the fix.
- **The English-only content gate is repaired, and its authoritative check now lives in a Go test.** It ran `grep` with `-P` and `-E` together, flags `grep` refuses to combine, and the resulting failure was swallowed three ways (`2>/dev/null`, `|| true`, `set +e`) — so the job always reported success without reading a file, on every pull request including every one that shipped in v5.0.0. `grep`'s exit status is now inspected explicitly, and the real check moved into `go test ./...`, which cannot pass vacuously. Its exemptions are line-scoped and carry a stated reason now, instead of exempting a whole file to protect one legitimate line.
- **Four prompts stopped forcing a fixed Spanish question regardless of the user's own language.** They asked "¿Quieres documentar el diseño de la base de datos?" or "¿Continuamos con el siguiente paso?" even mid-English-conversation, contradicting a rule the same files already stated elsewhere: ask in the language the user is using. All four now defer to that one rule. Spanish phrases the agent must still recognise in the user's own input (trigger phrases like "diseño de BD") are unaffected — losing those to a language gate would be the gate breaking the feature it guards.
- **`doc-reader` no longer hardcodes an in-project path, and now installs in every mode.** It hardcoded `docs/doc-agent/agent_sdd_context_project/` and bare context filenames, which is only correct in in-project mode — a vault keeps the same files under `<vault>/<node>/` with a `<node>_` prefix. Because the skill could not be correct in vault mode, it was gated to in-project installs, and switching an install to vault mode deleted it outright: vault users never received the skill at all. It now asks `status` for its resolved paths and reads exactly what comes back, which is correct in both modes because the program is what resolved them. The install-time gate, its sweep, and the `conditionalSkills` mechanism behind them are removed.
- **Six skills stopped instructing the write the program now performs.** Their role files absorbed v5's write-ownership model; the skill files did not — and a role tells the agent to read its skill first, so the stale instruction was the one read first, contradicting the role that had just loaded it. `doc-ddd`, `doc-idea`, `doc-pti`, `doc-refinement`, `doc-tech`, and `doc-to-sdd` no longer tell the agent to write, edit, or treat a file's existence as the completion condition. Descriptive content — naming tables, output paths — stays; only the imperative to produce the file is gone.
- **`doc-to-sdd` stops instead of hand-writing a context it could not submit.** It is the one phase executor that submits from its own protocol rather than the shared template, so it never inherited the shared unreachable-binary stop — its documented steps covered a clean write or a refused submission, and nothing else. A hand-written context file has no manifest, no source fingerprints, and no freshness record, so nothing downstream could tell it apart from a real one or know it had gone stale.
