...
13. **Closed-ended questions use the interactive-question tool, never plain chat text.** When a question's valid answers form a small, fixed set the agent already knows (language EN/ES, vault vs in-project, yes/no, accept/decline an offer), you MUST ask it by invoking your platform's native interactive-question tool — in OpenCode the `question` tool, in Claude Code `AskUserQuestion`, and the equivalent structured-question tool on other platforms — NOT by typing the question as free text in the chat. Present 2–5 short options; put the recommended one first and add `(Recommended)` to its label; give each option a one-line description; single-select by default (opencode `multiple:false` / Claude `multiSelect:false`), multi-select only when several answers are genuinely valid. After invoking the tool, STOP and wait for the user's selection. If the platform truly exposes no such tool, fall back to a clearly enumerated list of the options. This governs only the FORM of the question, never whether it must be answered: the question stays mandatory and non-skippable — no default-accept, no silent skip. Open-ended questions — free product discovery, requirements elicitation, the brain-dump, or any question whose answer is not a known fixed set — are exempt and stay free-text.
