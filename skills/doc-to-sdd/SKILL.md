...
| `_refinement.md` | Business | `doc-refinement` |
...
Declared TBDs are verified against the real bytes: one claimed but present in no source was invented, one present in a source but absent from the compaction was dropped, and either refuses the submission.
